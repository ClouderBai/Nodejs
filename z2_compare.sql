...
glbl_emply_id,
    ...
    dprt_cost_cntr_nbr,
    dprt_cost_cntr_nm,
    case when hcm.role_org_path is null and dm.dsm_glgl_emply_id is not null THEN 
        ((((((('MR'::text || ':='::text) || dm.bu_name::text) || ','::text) || dm.area_name::text) || ','::text) || dm.rgn_name::text) || ','::text) || dm.dstrct_name::text
        WHEN hcm.role_org_path is null and rds.rsd_glgl_emply_id is not null then
            (((((('MR'::text || ':='::text) || rds.bu_name::text) || ','::text) || rds.area_name::text) || ','::text) || rds.rgn_name::text) || ','
    end as role_org_path
     from employees1 hcm
LEFT JOIN ( SELECT DISTINCT vm_sales_org.bu_name,
                    vm_sales_org.buhead_glgl_emply_id,
                    ...
                    vm_sales_org.dsm_glgl_emply_id
                   FROM cmd_owner.vm_sales_org
                  WHERE vm_sales_org.dsm_glgl_emply_id IS NOT NULL AND vm_sales_org.dsm_glgl_emply_id <> 0) dm
    ON hcm.role='MR' AND role_org_path is NULL AND dm.dsm_glgl_emply_id::varchar = hcm.sprvsr_glbl_id
...
LEFT JOIN ( SELECT DISTINCT vm_sales_org.bu_name,
                    ...
                    vm_sales_org.rsd_glgl_emply_id
                   FROM cmd_owner.vm_sales_org
                  WHERE vm_sales_org.rsd_glgl_emply_id IS NOT NULL AND vm_sales_org.rsd_glgl_emply_id <> 0
    ) rds ON hcm.role='MR' AND role_org_path is NULL AND rds.rsd_glgl_emply_id::varchar  = hcm.sprvsr_glbl_id

)

 SELECT glbl_emply_id,
    role,
    acnt_name,
    chinese_name,
    email,
   ...
   FROM employees
-- WHERE glbl_emply_id in (2274964,3023617)
